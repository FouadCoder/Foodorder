          ...
          body: Container(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  // Text Address
                  const Text("Delivery Address"  , style: TextStyle(fontWeight: FontWeight.bold , fontSize: 20),),
                ...
                child: Lottie.asset(
                  ...
                  TextboxAuth(textfield: "Note for Delivery", controller: noteAddress, color: Colors.white),
                  ...
                  RadioListTile(
                    title: Row(
                      ...
                      const Text("Payment upon delivery" , style: TextStyle(fontSize: 15 , fontWeight: FontWeight.w500),),
                    ...
                    value: "Cash", groupValue: payment,onChanged: (val){ setState(() { payment = val; paymentselected = true;});}),
                    // Visa
                  RadioListTile(
                    title: Row(
                      mainAxisAlignment: MainAxisAlignment.end,
                      children: [
                        // Text for the payment way
                      const Text("Credit card" , style: TextStyle(fontSize: 15 , fontWeight: FontWeight.w500),),
                      const SizedBox(width: 15),
                      // Photo like Cash or Vias
                      SizedBox(
                        height: 25,
                    ...
                    activeColor: brownColor,
                    value: "Credit card", groupValue: payment, onChanged: (val){ setState(() { payment = val; paymentselected = true;});}),
                    ...
                    // Build here to get the price from Cubit
                      ...
                      builder: (context , state) {
                        if(state is CountTotalLoading){
                          return const Center(child: Text("Calculating the total ...."));
                        }
                          ...
                          return Column(
                            children: [
                              TwotextOneline(leftText: "Total: ", rightText: "\$$totalprice"),
                              // For the Price of delivery 
                              const SizedBox(height: 25),    
                              TwotextOneline(leftText: "Delivery: ", rightText: "\$$deliveryprice"),
                              const SizedBox(height: 20),
                              Container(height: 2, color: Colors.grey,),
                              const SizedBox(height: 20),
                              // For Text Grand Total
                              TwotextOneline(leftText: "Grand Total: ", rightText: "\$$grandTotal"),
                              const SizedBox(height: 30),
                            ],
                          );
                        }
                      ...
                      child: BlocListener<OrderFoodCubit,OrderFoodState>(
                        listener: (context , state) {
                          if(state is OrderFoodLoaded){
                            ...
                            showdialogSuccessOrder(context, "Your order has been placed successfully. Thank you for choosing our service ❤️" , );
                          } else if (state is OrderFoodError){
                            // if there any error in order 
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text("We're sorry, but we're unable to process your order at this time. Please try again later.") , backgroundColor: redC,)
                            );
                          }
                        }// Child Listener 
                        , child: NormalButton(textbutton: "ORDER NOW", backgroundColor: brownColor, onPressed: (){
                          if(paymentselected == true){ // to see if he choose payment or not 
                          ...
                          if(noteAddress.text.isNotEmpty){
                            ...
                            context.read<OrderFoodCubit>().sendTheOrder(name, email , address , note , paymentWay!);
                            
                          }
                          // else if paymentselectd == false
                          else{
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text("Please choose a payment method such as cash or Credit card.") , backgroundColor: redC,)
                            );
                          }
                          }),
                      ))
