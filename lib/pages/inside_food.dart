...
class InsideFood extends StatefulWidget  {
  ...
  const InsideFood({super.key, required this.foodItems, required this.indexfood});
  @override
  State<InsideFood> createState() => _InsideFoodState();
}

class _InsideFoodState extends State<InsideFood> with SingleTickerProviderStateMixin{
          ...
          body: SingleChildScrollView(
            child: Container(
              margin: EdgeInsets.symmetric(
                horizontal: MediaQuery.of(context).size.width * 0.02, // 2% OF SCREEN
              ...
              child: Column(
                ...
                AspectRatio(
                  ...
                  // Text description 
                  Text("${widget.foodItems[widget.indexfood]["description2"]}" , 
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        // Price
                        child:Center(child: Text("\$${widget.foodItems[widget.indexfood]["price"]}" , style: const TextStyle(fontSize: 16 , color: Colors.white , fontWeight: FontWeight.bold),))),
                      ...
                      BlocListener<AddItemCartCubit , AddItemCartState>(
                        listener: (context , state){
                          ...
                          if(state is AddItemCartLoaded){
                            showDialogCart(context, "The item is now in your cart. Happy shopping!");
                          }
                          // If there any error , this one wil work
                          else if(state is AddItemCartError){
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text("Failed to add item to cart. Please try again later." ) , backgroundColor: Colors.red,)
                            );
                          }
                        },
                        child: Expanded(
                          child: NormalButton(textbutton: "Add to cart", backgroundColor: brownColor,
                        onPressed: (){
                          String name = "${widget.foodItems[widget.indexfood]["name"]}";
                          String idFood = "${widget.foodItems[widget.indexfood]["id"]}";
                          String category = "${widget.foodItems[widget.indexfood]["category"]}";
                          String urlImage = "${widget.foodItems[widget.indexfood]["URLimage"]}";
                          int quantity = 1;
                          context.read<AddItemCartCubit>().addNewItemToCart(idFood , name , category , quantity , urlImage);
                            } ,)),
                      )
                    ],
                  ) 
